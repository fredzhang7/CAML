...
def get_model(args, fe_metadata, device):
  ...
  if args.model == 'CAML':
    if 'openai' in args.fe_type or 'lvd142m' in args.fe_type or 'eva02' in args.fe_type or '.dino' in args.fe_type:
...
def get_test_path(args, data_path):
  ...
  if args.eval_dataset == 'meta_iNat':
    ...
    test_path = os.path.join(data_path, args.eval_dataset + '/test')
    if not os.path.exists(test_path):
      raise Exception(f'The specified test path does not exist: {test_path}')
  return test_path
